...
### Распространяется на свободной основе на базе лицензии 
###
import os
...
# for_script/Carwash 2005 vx520.xml
...
try:
    ...
    pool3_pos = sline_searching(arrows, pool3, data_dict.get('unipos'))
except TypeError:
    print('Нет 3-го пула')
    pool3_pos = None
try:
...
for line in edited_data:
    ...
    if line[-1] == "АЗС" and pool3_pos is not None:
        for i in range(0, 324):
            arrows.remove(f'{pool3_pos + i}')
    ...
    if name is not None:
        ...
        arrows[adr + 3] = f'            <value>{line[2]}</value>'
        if line[-4] != 'NAN':
            arrows[brnd + 3] = f'            <value>{space_fill(line[-4])}{line[-4]}</value>'
